# image-generation-mcp Makefile — Go training wheels for the Python-brained
#
...
LDFLAGS := -X github.com/shotah/image-generation-mcp/server.ServerVersion=$(VERSION)
CGO_ENABLED ?= 0
...
cli: ## Build the MCP binary into ./bin/image-generation-mcp
	mkdir -p bin
	CGO_ENABLED=$(CGO_ENABLED) go build -ldflags "$(LDFLAGS)" -o bin/$(BINARY)$(EXE) .

install: ## Install binary into $$GOPATH/bin (or $$GOBIN) as image-generation-mcp
